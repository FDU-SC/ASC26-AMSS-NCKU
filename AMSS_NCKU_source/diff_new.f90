...
real*8 function fetch_bd(func, SoA, i_in, j_in, k_in)
  implicit none
  integer, intent(in) :: i_in, j_in, k_in
  real*8, dimension(:,:,:), intent(in) :: func
  integer, dimension(3), intent(in) :: SoA
  ...
  if (i <= 0) then; i = 2 - i; s = s * SoA(1); endif
  if (j <= 0) then; j = 2 - j; s = s * SoA(2); endif
  if (k <= 0) then; k = 2 - k; s = s * SoA(3); endif

  fetch_bd = func(i, j, k) * s
end function
  ...
  real*8,  dimension(ex(1),ex(2),ex(3)), intent(out):: fx,fy,fz
  real*8,                                intent(in) :: X(ex(1)),Y(ex(2)),Z(ex(3))
  integer,                               intent(in ):: SYM1,SYM2,SYM3

!~~~~~~ other variables

  real*8 :: dX,dY,dZ
  integer, dimension(3) :: SoA
  integer :: imin,jmin,kmin,imax,jmax,kmax,i,j,k
...
!  fx(i) = -----------------------------------------------------------------
!                                        60 dx
      fx(i,j,k)=d60dx*(-fetch_bd(f, SoA, i-3,j,k)+F9*fetch_bd(f, SoA, i-2,j,k)-F45*fetch_bd(f, SoA, i-1,j,k)+F45*fetch_bd(f, SoA, i+1,j,k)-F9*fetch_bd(f, SoA, i+2,j,k)+fetch_bd(f, SoA, i+3,j,k))
...
!  fx(i) = ---------------------------------------------
!                             12 dx
      fx(i,j,k)=d12dx*(fetch_bd(f, SoA, i-2,j,k)-EIT*fetch_bd(f, SoA, i-1,j,k)+EIT*fetch_bd(f, SoA, i+1,j,k)-fetch_bd(f, SoA, i+2,j,k))
...
!  fx(i) = --------------------------------
!                     2 dx
      fx(i,j,k)=d2dx*(-fetch_bd(f, SoA, i-1,j,k)+fetch_bd(f, SoA, i+1,j,k))
! set imax and imin 0
    else
      fx(i,j,k) = ZEO
    endif
! y direction   
      ...
      fy(i,j,k)=d60dy*(-fetch_bd(f, SoA, i,j-3,k)+F9*fetch_bd(f, SoA, i,j-2,k)-F45*fetch_bd(f, SoA, i,j-1,k)+F45*fetch_bd(f, SoA, i,j+1,k)-F9*fetch_bd(f, SoA, i,j+2,k)+fetch_bd(f, SoA, i,j+3,k))

    elseif(j+2 <= jmax .and. j-2 >= jmin)then

      fy(i,j,k)=d12dy*(fetch_bd(f, SoA, i,j-2,k)-EIT*fetch_bd(f, SoA, i,j-1,k)+EIT*fetch_bd(f, SoA, i,j+1,k)-fetch_bd(f, SoA, i,j+2,k))

    elseif(j+1 <= jmax .and. j-1 >= jmin)then

      fy(i,j,k)=d2dy*(-fetch_bd(f, SoA, i,j-1,k)+fetch_bd(f, SoA, i,j+1,k))
    else
      fy(i,j,k) = ZEO
...
! z direction   
      ...
      fz(i,j,k)=d60dz*(-fetch_bd(f, SoA, i,j,k-3)+F9*fetch_bd(f, SoA, i,j,k-2)-F45*fetch_bd(f, SoA, i,j,k-1)+F45*fetch_bd(f, SoA, i,j,k+1)-F9*fetch_bd(f, SoA, i,j,k+2)+fetch_bd(f, SoA, i,j,k+3))
    elseif(k+2 <= kmax .and. k-2 >= kmin)then

      fz(i,j,k)=d12dz*(fetch_bd(f, SoA, i,j,k-2)-EIT*fetch_bd(f, SoA, i,j,k-1)+EIT*fetch_bd(f, SoA, i,j,k+1)-fetch_bd(f, SoA, i,j,k+2))

    elseif(k+1 <= kmax .and. k-1 >= kmin)then

      fz(i,j,k)=d2dz*(-fetch_bd(f, SoA, i,j,k-1)+fetch_bd(f, SoA, i,j,k+1))
    else
      fz(i,j,k) = ZEO
...
!  fxx(i) = -----------------------------------------------------------------------------------
!                                                180 dx^2 
   fxx(i,j,k) = Xdxdx*(TWO*fetch_bd(f, SoA, i-3,j,k) -F27*fetch_bd(f, SoA, i-2,j,k)+F270*fetch_bd(f, SoA, i-1,j,k)-F490*fetch_bd(f, SoA, i,j,k) &
                      +TWO*fetch_bd(f, SoA, i+3,j,k)-F27*fetch_bd(f, SoA, i+2,j,k)+F270*fetch_bd(f, SoA, i+1,j,k)               )
   elseif(i+2 <= imax .and. i-2 >= imin)then
!
!               - f(i-2) + 16 f(i-1) - 30 f(i) + 16 f(i+1) - f(i+2)
!  fxx(i) = ----------------------------------------------------------
!                                  12 dx^2 
   fxx(i,j,k) = Fdxdx*(-fetch_bd(f, SoA, i-2,j,k)+F16*fetch_bd(f, SoA, i-1,j,k)-F30*fetch_bd(f, SoA, i,j,k) &
                       -fetch_bd(f, SoA, i+2,j,k)+F16*fetch_bd(f, SoA, i+1,j,k)              )
   elseif(i+1 <= imax .and. i-1 >= imin)then
!
!               f(i-1) - 2 f(i) + f(i+1)
!  fxx(i) = --------------------------------
!                         dx^2 
   fxx(i,j,k) = Sdxdx*(fetch_bd(f, SoA, i-1,j,k)-TWO*fetch_bd(f, SoA, i,j,k) &
                      +fetch_bd(f, SoA, i+1,j,k)              )
   else
    fxx(i,j,k) = ZEO
   endif
...
!~~~~~~ fyy
   ...
   fyy(i,j,k) = Xdydy*(TWO*fetch_bd(f, SoA, i,j-3,k)-F27*fetch_bd(f, SoA, i,j-2,k)+F270*fetch_bd(f, SoA, i,j-1,k)-F490*fetch_bd(f, SoA, i,j,k) &
                      +TWO*fetch_bd(f, SoA, i,j+3,k)-F27*fetch_bd(f, SoA, i,j+2,k)+F270*fetch_bd(f, SoA, i,j+1,k)               )
   elseif(j+2 <= jmax .and. j-2 >= jmin)then

   fyy(i,j,k) = Fdydy*(-fetch_bd(f, SoA, i,j-2,k)+F16*fetch_bd(f, SoA, i,j-1,k)-F30*fetch_bd(f, SoA, i,j,k) &
                       -fetch_bd(f, SoA, i,j+2,k)+F16*fetch_bd(f, SoA, i,j+1,k)              )
   elseif(j+1 <= jmax .and. j-1 >= jmin)then

   fyy(i,j,k) = Sdydy*(fetch_bd(f, SoA, i,j-1,k)-TWO*fetch_bd(f, SoA, i,j,k) &
                      +fetch_bd(f, SoA, i,j+1,k)              )
  else
    fyy(i,j,k) = ZEO
   endif
...
!~~~~~~ fzz
   ...
   fzz(i,j,k) = Xdzdz*(TWO*fetch_bd(f, SoA, i,j,k-3)-F27*fetch_bd(f, SoA, i,j,k-2)+F270*fetch_bd(f, SoA, i,j,k-1)-F490*fetch_bd(f, SoA, i,j,k) &
                      +TWO*fetch_bd(f, SoA, i,j,k+3)-F27*fetch_bd(f, SoA, i,j,k+2)+F270*fetch_bd(f, SoA, i,j,k+1)               )
   elseif(k+2 <= kmax .and. k-2 >= kmin)then

   fzz(i,j,k) = Fdzdz*(-fetch_bd(f, SoA, i,j,k-2)+F16*fetch_bd(f, SoA, i,j,k-1)-F30*fetch_bd(f, SoA, i,j,k) &
                       -fetch_bd(f, SoA, i,j,k+2)+F16*fetch_bd(f, SoA, i,j,k+1)              )
   elseif(k+1 <= kmax .and. k-1 >= kmin)then

   fzz(i,j,k) = Sdzdz*(fetch_bd(f, SoA, i,j,k-1)-TWO*fetch_bd(f, SoA, i,j,k) &
                      +fetch_bd(f, SoA, i,j,k+1)              )
   else
    fzz(i,j,k) = ZEO
   endif
!~~~~~~ fxy
...
!  fxy(i,j) = ------------------------------------------------------------------------------------------------
!                                                          3600 dx dy
   fxy(i,j,k) = Xdxdy*(-    (-fetch_bd(f, SoA, i-3,j-3,k)+F9*fetch_bd(f, SoA, i-2,j-3,k)-F45*fetch_bd(f, SoA, i-1,j-3,k)+F45*fetch_bd(f, SoA, i+1,j-3,k)-F9*fetch_bd(f, SoA, i+2,j-3,k)+fetch_bd(f, SoA, i+3,j-3,k))  &
                       +F9 *(-fetch_bd(f, SoA, i-3,j-2,k)+F9*fetch_bd(f, SoA, i-2,j-2,k)-F45*fetch_bd(f, SoA, i-1,j-2,k)+F45*fetch_bd(f, SoA, i+1,j-2,k)-F9*fetch_bd(f, SoA, i+2,j-2,k)+fetch_bd(f, SoA, i+3,j-2,k))  &
                       -F45*(-fetch_bd(f, SoA, i-3,j-1,k)+F9*fetch_bd(f, SoA, i-2,j-1,k)-F45*fetch_bd(f, SoA, i-1,j-1,k)+F45*fetch_bd(f, SoA, i+1,j-1,k)-F9*fetch_bd(f, SoA, i+2,j-1,k)+fetch_bd(f, SoA, i+3,j-1,k))  &
                       +F45*(-fetch_bd(f, SoA, i-3,j+1,k)+F9*fetch_bd(f, SoA, i-2,j+1,k)-F45*fetch_bd(f, SoA, i-1,j+1,k)+F45*fetch_bd(f, SoA, i+1,j+1,k)-F9*fetch_bd(f, SoA, i+2,j+1,k)+fetch_bd(f, SoA, i+3,j+1,k))  &
                       -F9 *(-fetch_bd(f, SoA, i-3,j+2,k)+F9*fetch_bd(f, SoA, i-2,j+2,k)-F45*fetch_bd(f, SoA, i-1,j+2,k)+F45*fetch_bd(f, SoA, i+1,j+2,k)-F9*fetch_bd(f, SoA, i+2,j+2,k)+fetch_bd(f, SoA, i+3,j+2,k))  &
                       +    (-fetch_bd(f, SoA, i-3,j+3,k)+F9*fetch_bd(f, SoA, i-2,j+3,k)-F45*fetch_bd(f, SoA, i-1,j+3,k)+F45*fetch_bd(f, SoA, i+1,j+3,k)-F9*fetch_bd(f, SoA, i+2,j+3,k)+fetch_bd(f, SoA, i+3,j+3,k)))
...
!  fxy(i,j) = ----------------------------------------------------------------
!                                  144 dx dy
   fxy(i,j,k) = Fdxdy*(     (fetch_bd(f, SoA, i-2,j-2,k)-F8*fetch_bd(f, SoA, i-1,j-2,k)+F8*fetch_bd(f, SoA, i+1,j-2,k)-fetch_bd(f, SoA, i+2,j-2,k))  &
                       -F8 *(fetch_bd(f, SoA, i-2,j-1,k)-F8*fetch_bd(f, SoA, i-1,j-1,k)+F8*fetch_bd(f, SoA, i+1,j-1,k)-fetch_bd(f, SoA, i+2,j-1,k))  &
                       +F8 *(fetch_bd(f, SoA, i-2,j+1,k)-F8*fetch_bd(f, SoA, i-1,j+1,k)+F8*fetch_bd(f, SoA, i+1,j+1,k)-fetch_bd(f, SoA, i+2,j+1,k))  &
                       -    (fetch_bd(f, SoA, i-2,j+2,k)-F8*fetch_bd(f, SoA, i-1,j+2,k)+F8*fetch_bd(f, SoA, i+1,j+2,k)-fetch_bd(f, SoA, i+2,j+2,k)))

   elseif(i+1 <= imax .and. i-1 >= imin .and. j+1 <= jmax .and. j-1 >= jmin)then
!                 f(i-1,j-1) - f(i+1,j-1) - f(i-1,j+1) + f(i+1,j+1) 
!  fxy(i,j) = -----------------------------------------------------------
!                                      4 dx dy
   fxy(i,j,k) = Sdxdy*(fetch_bd(f, SoA, i-1,j-1,k)-fetch_bd(f, SoA, i+1,j-1,k)-fetch_bd(f, SoA, i-1,j+1,k)+fetch_bd(f, SoA, i+1,j+1,k))
   else
    fxy(i,j,k) = ZEO
   endif
!~~~~~~ fxz
   ...
   fxz(i,j,k) = Xdxdz*(-    (-fetch_bd(f, SoA, i-3,j,k-3)+F9*fetch_bd(f, SoA, i-2,j,k-3)-F45*fetch_bd(f, SoA, i-1,j,k-3)+F45*fetch_bd(f, SoA, i+1,j,k-3)-F9*fetch_bd(f, SoA, i+2,j,k-3)+fetch_bd(f, SoA, i+3,j,k-3))  &
                       +F9 *(-fetch_bd(f, SoA, i-3,j,k-2)+F9*fetch_bd(f, SoA, i-2,j,k-2)-F45*fetch_bd(f, SoA, i-1,j,k-2)+F45*fetch_bd(f, SoA, i+1,j,k-2)-F9*fetch_bd(f, SoA, i+2,j,k-2)+fetch_bd(f, SoA, i+3,j,k-2))  &
                       -F45*(-fetch_bd(f, SoA, i-3,j,k-1)+F9*fetch_bd(f, SoA, i-2,j,k-1)-F45*fetch_bd(f, SoA, i-1,j,k-1)+F45*fetch_bd(f, SoA, i+1,j,k-1)-F9*fetch_bd(f, SoA, i+2,j,k-1)+fetch_bd(f, SoA, i+3,j,k-1))  &
                       +F45*(-fetch_bd(f, SoA, i-3,j,k+1)+F9*fetch_bd(f, SoA, i-2,j,k+1)-F45*fetch_bd(f, SoA, i-1,j,k+1)+F45*fetch_bd(f, SoA, i+1,j,k+1)-F9*fetch_bd(f, SoA, i+2,j,k+1)+fetch_bd(f, SoA, i+3,j,k+1))  &
                       -F9 *(-fetch_bd(f, SoA, i-3,j,k+2)+F9*fetch_bd(f, SoA, i-2,j,k+2)-F45*fetch_bd(f, SoA, i-1,j,k+2)+F45*fetch_bd(f, SoA, i+1,j,k+2)-F9*fetch_bd(f, SoA, i+2,j,k+2)+fetch_bd(f, SoA, i+3,j,k+2))  &
                       +    (-fetch_bd(f, SoA, i-3,j,k+3)+F9*fetch_bd(f, SoA, i-2,j,k+3)-F45*fetch_bd(f, SoA, i-1,j,k+3)+F45*fetch_bd(f, SoA, i+1,j,k+3)-F9*fetch_bd(f, SoA, i+2,j,k+3)+fetch_bd(f, SoA, i+3,j,k+3)))
   elseif(i+2 <= imax .and. i-2 >= imin .and. k+2 <= kmax .and. k-2 >= kmin)then
   fxz(i,j,k) = Fdxdz*(     (fetch_bd(f, SoA, i-2,j,k-2)-F8*fetch_bd(f, SoA, i-1,j,k-2)+F8*fetch_bd(f, SoA, i+1,j,k-2)-fetch_bd(f, SoA, i+2,j,k-2))  &
                       -F8 *(fetch_bd(f, SoA, i-2,j,k-1)-F8*fetch_bd(f, SoA, i-1,j,k-1)+F8*fetch_bd(f, SoA, i+1,j,k-1)-fetch_bd(f, SoA, i+2,j,k-1))  &
                       +F8 *(fetch_bd(f, SoA, i-2,j,k+1)-F8*fetch_bd(f, SoA, i-1,j,k+1)+F8*fetch_bd(f, SoA, i+1,j,k+1)-fetch_bd(f, SoA, i+2,j,k+1))  &
                       -    (fetch_bd(f, SoA, i-2,j,k+2)-F8*fetch_bd(f, SoA, i-1,j,k+2)+F8*fetch_bd(f, SoA, i+1,j,k+2)-fetch_bd(f, SoA, i+2,j,k+2)))
   elseif(i+1 <= imax .and. i-1 >= imin .and. k+1 <= kmax .and. k-1 >= kmin)then
   fxz(i,j,k) = Sdxdz*(fetch_bd(f, SoA, i-1,j,k-1)-fetch_bd(f, SoA, i+1,j,k-1)-fetch_bd(f, SoA, i-1,j,k+1)+fetch_bd(f, SoA, i+1,j,k+1))
   else
    fxz(i,j,k) = ZEO
   endif
!~~~~~~ fyz
   ...
   fyz(i,j,k) = Xdydz*(-    (-fetch_bd(f, SoA, i,j-3,k-3)+F9*fetch_bd(f, SoA, i,j-2,k-3)-F45*fetch_bd(f, SoA, i,j-1,k-3)+F45*fetch_bd(f, SoA, i,j+1,k-3)-F9*fetch_bd(f, SoA, i,j+2,k-3)+fetch_bd(f, SoA, i,j+3,k-3))  &
                       +F9 *(-fetch_bd(f, SoA, i,j-3,k-2)+F9*fetch_bd(f, SoA, i,j-2,k-2)-F45*fetch_bd(f, SoA, i,j-1,k-2)+F45*fetch_bd(f, SoA, i,j+1,k-2)-F9*fetch_bd(f, SoA, i,j+2,k-2)+fetch_bd(f, SoA, i,j+3,k-2))  &
                       -F45*(-fetch_bd(f, SoA, i,j-3,k-1)+F9*fetch_bd(f, SoA, i,j-2,k-1)-F45*fetch_bd(f, SoA, i,j-1,k-1)+F45*fetch_bd(f, SoA, i,j+1,k-1)-F9*fetch_bd(f, SoA, i,j+2,k-1)+fetch_bd(f, SoA, i,j+3,k-1))  &
                       +F45*(-fetch_bd(f, SoA, i,j-3,k+1)+F9*fetch_bd(f, SoA, i,j-2,k+1)-F45*fetch_bd(f, SoA, i,j-1,k+1)+F45*fetch_bd(f, SoA, i,j+1,k+1)-F9*fetch_bd(f, SoA, i,j+2,k+1)+fetch_bd(f, SoA, i,j+3,k+1))  &
                       -F9 *(-fetch_bd(f, SoA, i,j-3,k+2)+F9*fetch_bd(f, SoA, i,j-2,k+2)-F45*fetch_bd(f, SoA, i,j-1,k+2)+F45*fetch_bd(f, SoA, i,j+1,k+2)-F9*fetch_bd(f, SoA, i,j+2,k+2)+fetch_bd(f, SoA, i,j+3,k+2))  &
                       +    (-fetch_bd(f, SoA, i,j-3,k+3)+F9*fetch_bd(f, SoA, i,j-2,k+3)-F45*fetch_bd(f, SoA, i,j-1,k+3)+F45*fetch_bd(f, SoA, i,j+1,k+3)-F9*fetch_bd(f, SoA, i,j+2,k+3)+fetch_bd(f, SoA, i,j+3,k+3)))
   elseif(j+2 <= jmax .and. j-2 >= jmin .and. k+2 <= kmax .and. k-2 >= kmin)then
   fyz(i,j,k) = Fdydz*(     (fetch_bd(f, SoA, i,j-2,k-2)-F8*fetch_bd(f, SoA, i,j-1,k-2)+F8*fetch_bd(f, SoA, i,j+1,k-2)-fetch_bd(f, SoA, i,j+2,k-2))  &
                       -F8 *(fetch_bd(f, SoA, i,j-2,k-1)-F8*fetch_bd(f, SoA, i,j-1,k-1)+F8*fetch_bd(f, SoA, i,j+1,k-1)-fetch_bd(f, SoA, i,j+2,k-1))  &
                       +F8 *(fetch_bd(f, SoA, i,j-2,k+1)-F8*fetch_bd(f, SoA, i,j-1,k+1)+F8*fetch_bd(f, SoA, i,j+1,k+1)-fetch_bd(f, SoA, i,j+2,k+1))  &
                       -    (fetch_bd(f, SoA, i,j-2,k+2)-F8*fetch_bd(f, SoA, i,j-1,k+2)+F8*fetch_bd(f, SoA, i,j+1,k+2)-fetch_bd(f, SoA, i,j+2,k+2)))
   elseif(j+1 <= jmax .and. j-1 >= jmin .and. k+1 <= kmax .and. k-1 >= kmin)then
   fyz(i,j,k) = Sdydz*(fetch_bd(f, SoA, i,j-1,k-1)-fetch_bd(f, SoA, i,j+1,k-1)-fetch_bd(f, SoA, i,j-1,k+1)+fetch_bd(f, SoA, i,j+1,k+1))
   else
    fyz(i,j,k) = ZEO
   endif 

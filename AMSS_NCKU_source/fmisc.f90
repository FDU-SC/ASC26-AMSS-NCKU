...
!~~~~~~> Other parameters:
 ...
 if(cxB(3)>0)then
  ...
  call polin3_uniform(ya,cx(1),cx(2),cx(3),f_int,ORDN)
...
!------------------------------------------------------------------------------
! interpolation on a uniform 0..ordn-1 stencil
!------------------------------------------------------------------------------
  subroutine lagrange_uniform_weights(x,w,ordn)
  ...
  integer :: i,j
  real*8 :: xi,xj,t0,t1,t2,t3,t4,t5
  ...
  if(ordn == 6)then
    ...
    t4=x-4.d0
    t5=x-5.d0
    w(1)= -t1*t2*t3*t4*t5/1.2d2
    w(2)=  t0*t2*t3*t4*t5/2.4d1
    w(3)= -t0*t1*t3*t4*t5/1.2d1
    w(4)=  t0*t1*t2*t4*t5/1.2d1
    w(5)= -t0*t1*t2*t3*t5/2.4d1
    w(6)=  t0*t1*t2*t3*t4/1.2d2
    return
  endif
  ...
  end subroutine lagrange_uniform_weights

  subroutine polin3_uniform(ya,x1,x2,x3,y,ordn)
  ...
  integer :: m
  real*8, dimension(1:ordn) :: wx,wy,wz,tmp1
  real*8, dimension(1:ordn,1:ordn) :: tmp2

  call lagrange_uniform_weights(x1,wx,ordn)
  call lagrange_uniform_weights(x2,wy,ordn)
  call lagrange_uniform_weights(x3,wz,ordn)

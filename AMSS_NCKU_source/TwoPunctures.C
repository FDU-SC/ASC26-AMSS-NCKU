...
/* eq. 5.8.7 and 5.8.8 at x = (5.8.4) of 2nd edition C++ NR */
{
  ...
  std::vector<double> cosn(4 * n);
  for (int i = 0; i < 4 * n; i++)
    cosn[i] = cos(Pion * i / 2.0);
  ...
  else
  {
    for (j = 0; j < n; j++)
    {
      ...
      for (k = 0; k < n; k++)
      {
        sum += u[k] * cosn[((2 * j + 1) * k) % (4 * n)] * isignum;
        isignum = -isignum;
      }
      c[j] = sum;
    }
...
/* a (slow) Fourier transform, seems to be just eq. 12.1.6 and 12.1.9 of C++ NR (2nd ed) */
{
  ...
  fac = 1. / M;
  Pi_fac = Pi * fac;
  std::vector<double> sinn(N),cosn(N);
  for(int i=0;i<N;++i){
    sinn[i] = sin(Pi_fac * i);
    cosn[i] = cos(Pi_fac * i);
  }
  if (inv == 0)
  {
    for (l = 0; l <= M; l++)
    {
      ...
      for (k = 0; k < N; k++)
      {
        a[l] += fac * u[k] * cosn[(l * k) % N]; // cos(x)
        if (l > 0 && l < M)
          b[l] += fac * u[k] * sinn[(l * k) % N]; // sin(x)
      }
    }
    u[0] = a[0];
    u[M] = a[M];
  ...
  else
  {
    ...
    for (k = 0; k < N; k++)
    {
      ...
      for (l = 1; l < M; l++)
      {
        u[k] += a[l] * cosn[(l * k) % N] + b[l] * sinn[(l * k) % N];
      }
      iy = -iy;
    }
  }
                         ...
                         double const *rhs, int const *ncols, int **cols, double **JFD)
{
  ...
  int threads_to_use = std::max(1, std::min({n3 / 2 + 1, n2 / 2 + 1, omp_get_max_threads()}));

...
constexpr size_t max_transfer_plan_cache_entries = 512;

struct TransferPlan
{
  std::vector<MyList<Parallel::gridseg> *> src;
  std::vector<MyList<Parallel::gridseg> *> dst;
  bool copy_meta_ready = false;
  std::vector<int> copy_send_base;
  std::vector<int> copy_recv_base;
  std::vector<int> copy_send_nodes;
  std::vector<int> copy_recv_nodes;
  std::vector<int> copy_unpack_nodes;

  TransferPlan(int cpusize) : src(cpusize, 0), dst(cpusize, 0), copy_send_base(cpusize, 0), copy_recv_base(cpusize, 0) {}
...
std::unordered_map<std::string, TransferPlan *> transfer_plan_cache;
std::unordered_map<std::string, std::vector<int>> prolong_recv_bytes_cache;

void append_key_bytes(std::string &key, const void *data, size_t size)
{
  key.append((const char *)data, size);
}

template <class T>
void append_key_value(std::string &key, const T &value)
{
  append_key_bytes(key, &value, sizeof(value));
}

void append_block_key(std::string &key, const Block *B)
{
  const uintptr_t ptr = (uintptr_t)B;
  append_key_value(key, ptr);
  append_key_value(key, B->rank);
  append_key_value(key, B->lev);
  append_key_bytes(key, B->shape, sizeof(B->shape));
  append_key_bytes(key, B->bbox, sizeof(B->bbox));
}

void append_patch_key(std::string &key, const Patch *Pat)
{
  const uintptr_t ptr = (uintptr_t)Pat;
  append_key_value(key, ptr);
  append_key_value(key, Pat->lev);
  append_key_bytes(key, Pat->shape, sizeof(Pat->shape));
  append_key_bytes(key, Pat->bbox, sizeof(Pat->bbox));
  append_key_bytes(key, Pat->lli, sizeof(Pat->lli));
  append_key_bytes(key, Pat->uui, sizeof(Pat->uui));

  int blocks = 0;
  for (MyList<Block> *BP = Pat->blb; BP; BP = BP->next)
  {
    blocks++;
    append_block_key(key, BP->data);
    if (BP == Pat->ble)
      break;
  }
  append_key_value(key, blocks);
}

void append_patch_list_key(std::string &key, MyList<Patch> *PatL)
{
  int patches = 0;
  while (PatL)
  {
    patches++;
    append_patch_key(key, PatL->data);
    PatL = PatL->next;
  }
  append_key_value(key, patches);
}

void clear_transfer_plan_cache()
{
  for (auto &entry : transfer_plan_cache)
    delete entry.second;
  transfer_plan_cache.clear();
  prolong_recv_bytes_cache.clear();
}

TransferPlan *lookup_transfer_plan(const std::string &key)
{
  auto it = transfer_plan_cache.find(key);
  return (it == transfer_plan_cache.end()) ? 0 : it->second;
}

void remember_transfer_plan(const std::string &key, TransferPlan *plan)
{
  if (transfer_plan_cache.size() >= max_transfer_plan_cache_entries)
    clear_transfer_plan_cache();
  transfer_plan_cache[key] = plan;
}

void mark_patch_ranks(Patch *Pat, std::vector<char> &active)
{
  if (!Pat)
    return;
  ...
  while (BP)
  {
    const int rank = BP->data->rank;
    if (rank >= 0 && rank < (int)active.size())
      active[rank] = 1;
    if (BP == Pat->ble)
      break;
    BP = BP->next;
...
void mark_patch_list_ranks(MyList<Patch> *PatL, std::vector<char> &active)
{
  while (PatL)
  {
    mark_patch_ranks(PatL->data, active);
    PatL = PatL->next;
  }
}

bool patch_bbox_overlap(const Patch *A, const Patch *B)
{
  for (int i = 0; i < dim; i++)
  {
    if (A->bbox[dim + i] < B->bbox[i] - TINY || B->bbox[dim + i] < A->bbox[i] - TINY)
      return false;
  }
  return true;
}

inline int gridseg_payload_size(const Parallel::gridseg *g, int var_count)
{
  return var_count * g->shape[0] * g->shape[1] * g->shape[2];
}

int count_var_pair(MyList<var> *VarList1, MyList<var> *VarList2, const char *where)
{
  int var_count = 0;
  ...
  if (varls || varld)
  {
    cout << where << ": var lists do not match." << endl;
    MPI_Abort(MPI_COMM_WORLD, 1);
  }
  return var_count;
}

void prepare_copy_transfer_meta(TransferPlan *plan, int myrank)
{
  if (plan->copy_meta_ready)
    return;

  const int cpusize = (int)plan->src.size();
  MyList<Parallel::gridseg> *srcg = plan->src[myrank];
  MyList<Parallel::gridseg> *dstg = plan->dst[myrank];
  while (srcg && dstg)
  {
    const int node = dstg->data->Bg->rank;
    if (node >= 0 && node < cpusize)
      plan->copy_send_base[node] += gridseg_payload_size(dstg->data, 1);
    srcg = srcg->next;
    dstg = dstg->next;
  }
  ...
  for (int node = 0; node < cpusize; node++)
  {
    ...
    while (srcg && dstg)
    {
      if (srcg->data->Bg->rank == node && dstg->data->Bg->rank == myrank)
        plan->copy_recv_base[node] += gridseg_payload_size(dstg->data, 1);
      srcg = srcg->next;
      dstg = dstg->next;
    }
  }

  if (plan->copy_send_base[myrank] != plan->copy_recv_base[myrank])
  {
    cout << "Parallel::transfer_plan: local send/receive length mismatch." << endl;
    MPI_Abort(MPI_COMM_WORLD, 1);
  }

  for (int node = 0; node < cpusize; node++)
  {
    if (plan->copy_send_base[node] > 0)
      plan->copy_send_nodes.push_back(node);
    if (plan->copy_recv_base[node] > 0)
    {
      plan->copy_recv_nodes.push_back(node);
      plan->copy_unpack_nodes.push_back(node);
    }
  }
  plan->copy_meta_ready = true;
}

void transfer_plan(TransferPlan *plan, MyList<var> *VarList1, MyList<var> *VarList2, int Symmetry)
{
  int myrank;
  MPI_Comm_rank(MPI_COMM_WORLD, &myrank);

  const int cpusize = (int)plan->src.size();
  const int var_count = count_var_pair(VarList1, VarList2, "Parallel::transfer_plan");
  prepare_copy_transfer_meta(plan, myrank);

  std::vector<double *> send_data(cpusize, 0), rec_data(cpusize, 0);
  std::vector<MPI_Request> reqs;
  reqs.reserve(plan->copy_send_nodes.size() + plan->copy_recv_nodes.size());

  if (plan->copy_recv_base[myrank] > 0)
  {
    const int length = plan->copy_recv_base[myrank] * var_count;
    rec_data[myrank] = new double[length];
    if (!rec_data[myrank])
    {
      cout << "out of memory when new in transfer_plan, place 1" << endl;
      MPI_Abort(MPI_COMM_WORLD, 1);
    }
    Parallel::data_packer(rec_data[myrank], plan->src[myrank], plan->dst[myrank], myrank, PACK, VarList1, VarList2, Symmetry);
  }

  for (size_t i = 0; i < plan->copy_recv_nodes.size(); i++)
  {
    const int node = plan->copy_recv_nodes[i];
    if (node == myrank)
      continue;
    const int length = plan->copy_recv_base[node] * var_count;
    rec_data[node] = new double[length];
    if (!rec_data[node])
    {
      cout << "out of memory when new in transfer_plan, place 2" << endl;
      MPI_Abort(MPI_COMM_WORLD, 1);
    }
    MPI_Request rq;
    MPI_Irecv((void *)rec_data[node], length, MPI_DOUBLE, node, 1, MPI_COMM_WORLD, &rq);
    reqs.push_back(rq);
  }

  for (size_t i = 0; i < plan->copy_send_nodes.size(); i++)
  {
    const int node = plan->copy_send_nodes[i];
    if (node == myrank)
      continue;
    const int length = plan->copy_send_base[node] * var_count;
    send_data[node] = new double[length];
    if (!send_data[node])
    {
      cout << "out of memory when new in transfer_plan, place 3" << endl;
      MPI_Abort(MPI_COMM_WORLD, 1);
    }
    Parallel::data_packer(send_data[node], plan->src[myrank], plan->dst[myrank], node, PACK, VarList1, VarList2, Symmetry);
    MPI_Request rq;
    MPI_Isend((void *)send_data[node], length, MPI_DOUBLE, node, 1, MPI_COMM_WORLD, &rq);
    reqs.push_back(rq);
  }
  ...
  for (size_t i = 0; i < plan->copy_unpack_nodes.size(); i++)
  {
    const int node = plan->copy_unpack_nodes[i];
    Parallel::data_packer(rec_data[node], plan->src[node], plan->dst[node], node, UNPACK, VarList1, VarList2, Symmetry);
  }
...
TransferPlan *cached_sync_patch_plan(Patch *Pat, int Symmetry, int cpusize)
{
  std::string key("sync-patch", 10);
  append_key_value(key, cpusize);
  append_key_value(key, Symmetry);
  append_patch_key(key, Pat);
  if (TransferPlan *cached = lookup_transfer_plan(key))
    return cached;

  TransferPlan *plan = new TransferPlan(cpusize);
  MyList<Parallel::gridseg> *dst = Parallel::build_ghost_gsl(Pat);
  std::vector<char> active_rank(cpusize, 0);
  mark_patch_ranks(Pat, active_rank);
  for (int node = 0; node < cpusize; node++)
  {
    if (!active_rank[node])
      continue;
    MyList<Parallel::gridseg> *src = Parallel::build_owned_gsl0(Pat, node);
    Parallel::build_gstl(src, dst, &plan->src[node], &plan->dst[node]);
    if (src)
      src->destroyList();
  }
  if (dst)
    dst->destroyList();
  remember_transfer_plan(key, plan);
  return plan;
}

TransferPlan *cached_sync_list_plan(MyList<Patch> *PatL, int Symmetry, int cpusize)
{
  std::string key("sync-list", 9);
  append_key_value(key, cpusize);
  append_key_value(key, Symmetry);
  append_patch_list_key(key, PatL);
  if (TransferPlan *cached = lookup_transfer_plan(key))
    return cached;

  TransferPlan *plan = new TransferPlan(cpusize);
  MyList<Parallel::gridseg> *dst = Parallel::build_buffer_gsl(PatL);
  std::vector<char> active_rank(cpusize, 0);
  mark_patch_list_ranks(PatL, active_rank);
  for (int node = 0; node < cpusize; node++)
  {
    if (!active_rank[node])
      continue;
    MyList<Parallel::gridseg> *src = Parallel::build_owned_gsl(PatL, node, 5, Symmetry);
    Parallel::build_gstl(src, dst, &plan->src[node], &plan->dst[node]);
    if (src)
      src->destroyList();
  }
...
TransferPlan *cached_outbd_pair_plan(Patch *Patc, Patch *Patf, int Symmetry, int cpusize, int myrank)
{
  std::string key("outbd-pair", 10);
  append_key_value(key, cpusize);
  append_key_value(key, myrank);
  append_key_value(key, Symmetry);
  append_patch_key(key, Patc);
  append_patch_key(key, Patf);
  if (TransferPlan *cached = lookup_transfer_plan(key))
    return cached;

  TransferPlan *plan = new TransferPlan(cpusize);
  MyList<Parallel::gridseg> *dst = Parallel::build_buffer_gsl(Patf);
  MyList<Parallel::gridseg> *src = Parallel::build_owned_gsl4(Patc, myrank, Symmetry);
  Parallel::build_gstl(src, dst, &plan->src[myrank], &plan->dst[myrank]);
  if (src)
    src->destroyList();
  if (dst)
    dst->destroyList();
...
int Parallel::data_packer_prolong(double *data, MyList<Parallel::gridseg> *src, MyList<Parallel::gridseg> *dst, int rank_in, int dir,
                          MyList<var> *VarLists /* source */, MyList<var> *VarListd /* target */, int Symmetry)
{
  int myrank;
  MPI_Comm_rank(MPI_COMM_WORLD, &myrank);
                        ...
                        int Symmetry)
{
  ...
  std::vector<int> send_length(cpusize, 0), recv_length(cpusize, 0);
  MyList<Parallel::gridseg> *srcg = src[myrank];
  MyList<Parallel::gridseg> *dstg = dst[myrank];
  while (srcg && dstg)
  {
    const int node = dstg->data->Bg->rank;
    if (node >= 0 && node < cpusize)
      send_length[node] += gridseg_payload_size(dstg->data, var_count);
    srcg = srcg->next;
    dstg = dstg->next;
  }
  for (int node = 0; node < cpusize; node++)
  {
    ...
    {
      if (srcg->data->Bg->rank == node && dstg->data->Bg->rank == myrank)
        recv_length[node] += gridseg_payload_size(dstg->data, var_count);
      srcg = srcg->next;
      dstg = dstg->next;
    }
  }
  if (send_length[myrank] != recv_length[myrank])
  {
    cout << "Parallel::transfer: local send/receive length mismatch." << endl;
    MPI_Abort(MPI_COMM_WORLD, 1);
  }

  std::vector<double *> send_data(cpusize, 0), rec_data(cpusize, 0);
  std::vector<MPI_Request> reqs;
  reqs.reserve(2 * cpusize);

  if (send_length[myrank])
  {
    rec_data[myrank] = new double[send_length[myrank]];
    if (!rec_data[myrank])
    {
  ...
  for (int node = 0; node < cpusize; node++)
  {
    if (node == myrank || !recv_length[node])
      continue;
    rec_data[node] = new double[recv_length[node]];
    if (!rec_data[node])
    {
    ...
    MPI_Request rq;
    MPI_Irecv((void *)rec_data[node], recv_length[node], MPI_DOUBLE, node, 1, MPI_COMM_WORLD, &rq);
    reqs.push_back(rq);
  }

  for (int node = 0; node < cpusize; node++)
  {
    if (node == myrank || !send_length[node])
      continue;
    send_data[node] = new double[send_length[node]];
    if (!send_data[node])
    {
    ...
    MPI_Request rq;
    MPI_Isend((void *)send_data[node], send_length[node], MPI_DOUBLE, node, 1, MPI_COMM_WORLD, &rq);
    reqs.push_back(rq);
  }
...
void Parallel::transfer_prolong(MyList<Parallel::gridseg> **src, MyList<Parallel::gridseg> **dst,
                        MyList<var> *VarList1 /* source */, MyList<var> *VarList2 /*target */,
                        int Symmetry)
{
  int myrank, cpusize;
  ...
  std::vector<int> src_length(cpusize), dst_length(cpusize);
  auto dstg = dst[myrank];
  while (dstg) {
    dst_length[dstg->data->Bg->rank] += var_count * dstg->data->shape[0] *
                                        dstg->data->shape[1] *
                                        dstg->data->shape[2];
    dstg = dstg->next;
  }
  for (int node = 0; node < cpusize; ++node) {
    auto dstg = dst[node];
    while (dstg) {
      if (dstg->data->Bg->rank == myrank) {
        src_length[node] += var_count * dstg->data->shape[0] *
                            dstg->data->shape[1] * dstg->data->shape[2];
      }
      dstg = dstg->next;
    }
  }
  ...
  for(int node = 0; node< cpusize;node++){
    if (dst_length[node])
    {
      send_data[node] = new double[dst_length[node]];
      int DIM = dim;
      int size_out = 0;
      MyList<var> *varls, *varld;
      auto srcg = src[myrank];
      auto dstg = dst[myrank];
      while (srcg && dstg)
      {
        if (dstg->data->Bg->rank == node)
        {
          varls = VarList1;
          varld = VarList2;
          while (varls && varld)
          {
            tasks.push_back(Task{
              .data=send_data[node]+size_out,
              .vars = varls->data,
              .src = srcg->data,
              .dst = dstg->data
            });
            size_out += dstg->data->shape[0] * dstg->data->shape[1] * dstg->data->shape[2];
            varls = varls->next;
            varld = varld->next;
          }
        }
...
#pragma omp parallel for schedule(dynamic)
  for(int i=0;i<tasks.size();i++){
    int DIM = dim;
    auto srcg = tasks[i].src;
    auto dstg = tasks[i].dst;
    auto var = tasks[i].vars;
    auto data = tasks[i].data;
    f_prolong3(DIM, srcg->Bg->bbox, srcg->Bg->bbox + dim, srcg->Bg->shape,
               srcg->Bg->fgfs[var->sgfn], dstg->llb, dstg->uub, dstg->shape,
               data, dstg->llb, dstg->uub, var->SoA, Symmetry);
  }
  ...
  for (int node = 0; node < cpusize; node++)
  {
    ...
    if (dst_length[node])
    {
      MPI_Isend((void *)send_data[node], dst_length[node], MPI_DOUBLE, node, 1, MPI_COMM_WORLD, reqs + req_no++);
    }
    // receive from cpu#node to this cpu
    if (src_length[node])
    {
      rec_data[node] = new double[src_length[node]];
      MPI_Irecv((void *)rec_data[node], src_length[node], MPI_DOUBLE, node, 1, MPI_COMM_WORLD, reqs + req_no++);
    }
  }
  ...
  for (int node = 0; node < cpusize; node++)
    if (rec_data[node])
      data_packer_prolong(rec_data[node], src[node], dst[node], node, UNPACK, VarList1, VarList2, Symmetry);
...
constexpr int prolong_fine_mpi_tag = 5937;

struct ProlongChunkHdr
{
  double llbc[3], uubc[3];
  double dst_llb[3], dst_uub[3];
  double fine_bbox[6];
  int extc[3];
  int dst_shape[3];
  int fine_shape[3];
...
inline int product3i(const int *s) { return s[0] * s[1] * s[2]; }

bool coarse_halo_geom(const Parallel::gridseg *srcg, double *llb_pack, double *uub_pack, int *shape_pack)
{
  Block *Bc = srcg->Bg;
  for (int i = 0; i < dim; i++)
  {
    double SH = Bc->getdX(i);
    llb_pack[i] = srcg->llb[i] - ghost_width * SH;
    uub_pack[i] = srcg->uub[i] + ghost_width * SH;
    if (llb_pack[i] < Bc->bbox[i])
      llb_pack[i] = Bc->bbox[i];
    if (uub_pack[i] > Bc->bbox[dim + i])
      uub_pack[i] = Bc->bbox[dim + i];
#ifdef Vertex
#ifdef Cell
#error Both Vertex and Cell are defined
#endif
    if (uub_pack[i] - llb_pack[i] < SH / 2)
      return false;
    shape_pack[i] = int((uub_pack[i] - llb_pack[i]) / SH + 0.4) + 1;
    if (shape_pack[i] < 1)
      return false;
#else
#ifdef Cell
    if (uub_pack[i] - llb_pack[i] < SH / 2)
      return false;
    shape_pack[i] = int((uub_pack[i] - llb_pack[i]) / SH + 0.4);
    if (shape_pack[i] < 1)
      return false;
#else
#error Not define Vertex nor Cell
...
Block *find_fine_block_for_prolong(Patch *Pat, const ProlongChunkHdr &hdr)
{
  if (!Pat)
  {
    cout << "transfer_prolong_fine_owned: null Patf" << endl;
    MPI_Abort(MPI_COMM_WORLD, 1);
  }
  for (MyList<Block> *w = Pat->blb; w; w = w->next)
  {
    Block *b = w->data;
    bool ok = true;
    for (int k = 0; k < 2 * dim && ok; k++)
    {
      double eps = b->getdX(k % dim) * 1e-10;
      if (!feq(b->bbox[k], hdr.fine_bbox[k], eps))
        ok = false;
    }
    for (int k = 0; k < dim && ok; k++)
    {
      if (b->shape[k] != hdr.fine_shape[k])
        ok = false;
    }
    if (ok)
      return b;
  ...
  cout << "transfer_prolong_fine_owned: fine Block not found in Patf" << endl;
  MPI_Abort(MPI_COMM_WORLD, 1);
  return nullptr;
}
...
void Parallel::transfer_prolong_fine_owned(Patch *Patf, MyList<Parallel::gridseg> **transfer_src,
                                           ...
                                           MyList<var> *VarList1, MyList<var> *VarList2, int Symmetry)
{
  ...
  int var_count = 0;
  for (MyList<var> *va = VarList1, *vb = VarList2; va && vb; va = va->next, vb = vb->next)
    var_count++;
  if ((VarList1 && !VarList2) || (!VarList1 && VarList2))
  {
    cout << "transfer_prolong_fine_owned: var list mismatch." << endl;
    MPI_Abort(MPI_COMM_WORLD, 1);
  }
  {
    MyList<var> *va = VarList1, *vb = VarList2;
    while (va && vb)
    {
      va = va->next;
      vb = vb->next;
    ...
    if (va || vb)
    {
      cout << "transfer_prolong_fine_owned: var list length mismatch." << endl;
      MPI_Abort(MPI_COMM_WORLD, 1);
    }
  }
  if (var_count < 1)
  {
    cout << "transfer_prolong_fine_owned: empty var lists." << endl;
    MPI_Abort(MPI_COMM_WORLD, 1);
  }

  std::vector<int> send_bytes(cpusize, 0), recv_bytes(cpusize, 0);
  std::vector<std::vector<char>> send_buf(cpusize);

  // One list walk O(L), not O(cpusize * L): previously each dest loop rescanned the full pair list.
  MyList<Parallel::gridseg> *srcg = transfer_src[myrank];
  MyList<Parallel::gridseg> *dstg = transfer_dst[myrank];
  while (srcg && dstg)
  {
    const int dest = dstg->data->Bg->rank;
    double llb_pack[dim], uub_pack[dim];
    int shape_pack[dim];
    if (!coarse_halo_geom(srcg->data, llb_pack, uub_pack, shape_pack))
    {
      cout << "transfer_prolong_fine_owned: empty coarse halo after clip (rank " << myrank << " -> " << dest << ")." << endl;
      MPI_Abort(MPI_COMM_WORLD, 1);
    }
    const int npack = product3i(shape_pack);
    send_bytes[dest] += (int)sizeof(ProlongChunkHdr) + var_count * npack * (int)sizeof(double);
    srcg = srcg->next;
    dstg = dstg->next;
  }

  std::vector<size_t> pack_off(cpusize, 0);
  for (int dest = 0; dest < cpusize; dest++)
  {
    if (send_bytes[dest] > 0)
      send_buf[dest].resize((size_t)send_bytes[dest]);
  }

  srcg = transfer_src[myrank];
  dstg = transfer_dst[myrank];
  while (srcg && dstg)
  {
    const int dest = dstg->data->Bg->rank;
    double llb_pack[dim], uub_pack[dim];
    int shape_pack[dim];
    coarse_halo_geom(srcg->data, llb_pack, uub_pack, shape_pack);
    const int npack = product3i(shape_pack);
    Block *fB = dstg->data->Bg;
    Block *cB = srcg->data->Bg;
    ProlongChunkHdr hdr{};
    memcpy(hdr.llbc, llb_pack, sizeof(llb_pack));
    memcpy(hdr.uubc, uub_pack, sizeof(uub_pack));
    memcpy(hdr.dst_llb, dstg->data->llb, sizeof(hdr.dst_llb));
    memcpy(hdr.dst_uub, dstg->data->uub, sizeof(hdr.dst_uub));
    memcpy(hdr.fine_bbox, fB->bbox, sizeof(hdr.fine_bbox));
    memcpy(hdr.extc, shape_pack, sizeof(shape_pack));
    memcpy(hdr.dst_shape, dstg->data->shape, sizeof(hdr.dst_shape));
    memcpy(hdr.fine_shape, fB->shape, sizeof(hdr.fine_shape));
    hdr.var_count = var_count;

    char *wp = send_buf[dest].data() + pack_off[dest];
    pack_off[dest] += sizeof(ProlongChunkHdr) + (size_t)var_count * (size_t)npack * sizeof(double);
    memcpy(wp, &hdr, sizeof(hdr));
    wp += sizeof(hdr);
    int DIM = dim;
    for (MyList<var> *varls = VarList1; varls; varls = varls->next)
    {
      f_copy(DIM, llb_pack, uub_pack, shape_pack, (double *)wp, cB->bbox, cB->bbox + dim, cB->shape,
             cB->fgfs[varls->data->sgfn], llb_pack, uub_pack);
      wp += npack * (int)sizeof(double);
    }
    srcg = srcg->next;
    dstg = dstg->next;
  }

  std::string recv_key("prolong-recv", 12);
  append_key_value(recv_key, myrank);
  append_key_value(recv_key, var_count);
  const uintptr_t src_key = (uintptr_t)transfer_src;
  const uintptr_t dst_key = (uintptr_t)transfer_dst;
  append_key_value(recv_key, src_key);
  append_key_value(recv_key, dst_key);
  auto recv_it = prolong_recv_bytes_cache.find(recv_key);
  if (recv_it == prolong_recv_bytes_cache.end())
  {
    MPI_Alltoall(send_bytes.data(), 1, MPI_INT, recv_bytes.data(), 1, MPI_INT, MPI_COMM_WORLD);
    prolong_recv_bytes_cache[recv_key] = recv_bytes;
  }
  else
  {
    recv_bytes = recv_it->second;
  }

  std::vector<std::vector<char>> recv_buf(cpusize);
  for (int r = 0; r < cpusize; r++)
  {
    if (r == myrank)
    {
      if (recv_bytes[r] != send_bytes[r])
      {
        cout << "transfer_prolong_fine_owned: local send/receive size mismatch." << endl;
        MPI_Abort(MPI_COMM_WORLD, 1);
      }
      if (recv_bytes[r] > 0)
        recv_buf[r].swap(send_buf[r]);
    }
    else if (recv_bytes[r] > 0)
      recv_buf[r].resize(recv_bytes[r]);
  }
  ...
  for (int r = 0; r < cpusize; r++)
  {
    if (r != myrank && recv_bytes[r] > 0)
    {
      MPI_Request rq;
      MPI_Irecv(recv_buf[r].data(), recv_bytes[r], MPI_BYTE, r, prolong_fine_mpi_tag, MPI_COMM_WORLD, &rq);
      reqs.push_back(rq);
    }
  }
  for (int r = 0; r < cpusize; r++)
  {
    if (r != myrank && send_bytes[r] > 0)
    {
      MPI_Request rq;
      MPI_Isend(send_buf[r].data(), send_bytes[r], MPI_BYTE, r, prolong_fine_mpi_tag, MPI_COMM_WORLD, &rq);
      reqs.push_back(rq);
    }
  }
  if (!reqs.empty())
  ...
  for (int r = 0; r < cpusize; r++)
  {
    if (recv_bytes[r] == 0)
      continue;
    const char *rp = recv_buf[r].data();
    const char *const end = rp + recv_bytes[r];
    while (rp < end)
    {
      if ((size_t)(end - rp) < sizeof(ProlongChunkHdr))
      {
        cout << "transfer_prolong_fine_owned: truncated message from rank " << r << endl;
        MPI_Abort(MPI_COMM_WORLD, 1);
      }
      ProlongChunkHdr hdr;
      memcpy(&hdr, rp, sizeof(hdr));
      rp += sizeof(hdr);
      if (hdr.var_count != var_count)
      {
        cout << "transfer_prolong_fine_owned: var_count mismatch from rank " << r << endl;
        MPI_Abort(MPI_COMM_WORLD, 1);
      }
      const int npack = product3i(hdr.extc);
      const int chunk_dbytes = var_count * npack * (int)sizeof(double);
      if ((size_t)(end - rp) < (size_t)chunk_dbytes)
      {
        cout << "transfer_prolong_fine_owned: truncated coarse payload from rank " << r << endl;
        MPI_Abort(MPI_COMM_WORLD, 1);
      }
      Block *fineB = find_fine_block_for_prolong(Patf, hdr);
      std::vector<double> fine_out((size_t)product3i(hdr.dst_shape));
      int DIM = dim;
      MyList<var> *varls = VarList1;
      MyList<var> *varld = VarList2;
      for (int vi = 0; vi < var_count; vi++)
      {
        double *coarse_line = (double *)rp;
        rp += npack * (int)sizeof(double);
        f_prolong3(DIM, hdr.llbc, hdr.uubc, hdr.extc, coarse_line, hdr.dst_llb, hdr.dst_uub, hdr.dst_shape,
                   fine_out.data(), hdr.dst_llb, hdr.dst_uub, varls->data->SoA, Symmetry);
        f_copy(DIM, fineB->bbox, fineB->bbox + dim, fineB->shape, fineB->fgfs[varld->data->sgfn], hdr.dst_llb,
               hdr.dst_uub, hdr.dst_shape, fine_out.data(), hdr.dst_llb, hdr.dst_uub);
        varls = varls->next;
        varld = varld->next;
      }
    }
...
void Parallel::Sync(Patch *Pat, MyList<var> *VarList, int Symmetry)
{
  ...
  TransferPlan *plan = cached_sync_patch_plan(Pat, Symmetry, cpusize);
  transfer_plan(plan, VarList, VarList, Symmetry);
}
void Parallel::Sync(MyList<Patch> *PatL, MyList<var> *VarList, int Symmetry)
{
  ...
  TransferPlan *plan = cached_sync_list_plan(PatL, Symmetry, cpusize);
  transfer_plan(plan, VarList, VarList, Symmetry);
}
                           ...
                           int Symmetry)
{
  ...
  if (!patch_bbox_overlap(Patc, Patf))
    return;
  ...
  TransferPlan *plan = cached_outbd_pair_plan(Patc, Patf, Symmetry, cpusize, myrank);

  // transfer_prolong(transfer_src, transfer_dst, VarList1, VarList2, Symmetry);
  transfer_prolong_fine_owned(Patf, plan->src.data(), plan->dst.data(), VarList1, VarList2, Symmetry);
}
                           ...
                           int Symmetry)
{
  ...
  std::vector<char> active_rank(cpusize, 0);
  mark_patch_list_ranks(PatcL, active_rank);
  for (int node = 0; node < cpusize; node++)
  {
    if (!active_rank[node])
      continue;

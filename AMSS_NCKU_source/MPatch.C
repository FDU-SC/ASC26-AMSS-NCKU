...
class ShellfScheduler{
public:
  Patch* patch;
  int NN;
  double **XX;
  std::vector<Block*> belongings;

  ShellfScheduler(Patch* patch, int NN, double **XX)
    : patch(patch), NN(NN), XX(XX)
  {
    belongings.resize(NN);
    int myrank;
    MPI_Comm_rank(MPI_COMM_WORLD, &myrank);
    ...
    std::vector<BlockBounds> block_bounds;
...
// BUT for cell structure the bbox is (-1.5,0.5) and (0.5,2.5), there is no
// missing region at all
#ifdef Vertex
#ifdef Cell
#error Both Cell and Vertex are defined
#endif
        bb.llb[i] = (feq(bb.BP->bbox[i], patch->bbox[i], DH[i] / 2))
                       ? bb.BP->bbox[i] + patch->lli[i] * DH[i]
                       : bb.BP->bbox[i] + (ghost_width - 0.5) * DH[i];
        bb.uub[i] = (feq(bb.BP->bbox[dim + i], patch->bbox[dim + i], DH[i] / 2))
                       ? bb.BP->bbox[dim + i] - patch->uui[i] * DH[i]
                       : bb.BP->bbox[dim + i] - (ghost_width - 0.5) * DH[i];
#else
#ifdef Cell
        bb.llb[i] = (feq(bb.BP->bbox[i], patch->bbox[i], DH[i] / 2))
                       ? bb.BP->bbox[i] + patch->lli[i] * DH[i]
                       : bb.BP->bbox[i] + ghost_width * DH[i];
        bb.uub[i] = (feq(bb.BP->bbox[dim + i], patch->bbox[dim + i], DH[i] / 2))
                       ? bb.BP->bbox[dim + i] - patch->uui[i] * DH[i]
                       : bb.BP->bbox[dim + i] - ghost_width * DH[i];
#else
#error Not define Vertex nor Cell
#endif
#endif
      }
      block_bounds.push_back(bb);
      if (Bp == patch->ble)
        break;
      Bp = Bp->next;
    }
    ...
    {
      for (int i = 0; i < dim; i++) {
        if (myrank == 0 && (XX[i][j] < patch->bbox[i] + patch->lli[i] * DH[i] ||
      ...
      bool notfind = true;
      for (size_t b = 0; notfind && b < block_bounds.size(); b++) // run along Blocks
      {
        const BlockBounds &bb = block_bounds[b];

        bool flag = true;
        for (int i = 0; i < dim; i++) {
          if (XX[i][j] - bb.llb[i] < -DH[i] / 2 || XX[i][j] - bb.uub[i] > DH[i] / 2) {
            flag = false;
            break;
          }
        }
  ...
  void view() {
    ...
    for(auto b:belongings){
      int rank = b->rank;
      if(rank>=taskcount.size()) taskcount.resize(rank+1,0);
      taskcount[rank]++;
    }
    std::cout<<belongings[0]->shape[0]*belongings[0]->shape[1]*belongings[0]->shape[2]<<endl;
    for(auto i:taskcount){
      cout<<i<<" ";
    }
    cout<<endl;
  }
  void run(double *Shellf, MyList<var> *VarList, int Symmetry) {
    int myrank;
    MPI_Comm_rank(MPI_COMM_WORLD, &myrank);
    ...
    std::vector<double> shellf(NN * num_var);
    struct Task {
      Block *BP;
      int j, k;
      var *data;
    ...
    tasks.reserve(NN * num_var);
    for(int j=0;j<NN;++j){
      if(belongings[j]->rank!=myrank) continue;
      auto varl = VarList;
      int k = 0;
      while (varl) // run along variables
      {
        tasks.push_back({belongings[j], j, k, varl->data});
        varl = varl->next;
        k++;
      }
    }
#pragma omp parallel for
    for (int i = 0; i < tasks.size(); ++i) {
      auto task = tasks[i];
      f_global_interp(task.BP->shape, task.BP->X[0], task.BP->X[1],
                      task.BP->X[2], task.BP->fgfs[task.data->sgfn],
                      shellf[task.j * num_var + task.k], XX[0][task.j],
                      XX[1][task.j], XX[2][task.j], ordn, task.data->SoA,
                      Symmetry);
    }
    MPI_Allreduce(shellf.data(), Shellf, NN * num_var, MPI_DOUBLE, MPI_SUM,
                  MPI_COMM_WORLD);
  }

  void run2(double *Shellf, MyList<var> *VarList, int Symmetry) {
    const int task_threshold = 4096;
    if(NN<task_threshold){
      run(Shellf, VarList, Symmetry);
      return;
    }
    ...
    allvars.reserve(16);
    for (auto p = VarList; p; p = p->next)
      allvars.push_back(p->data);

    const int nvar = static_cast<int>(allvars.size());

    /* ---------------- count tasks per rank ---------------- */
    std::vector<int> taskcount(world_size, 0);
    for (int j = 0; j < NN; ++j)
      taskcount[belongings[j]->rank]+= nvar;

    /* ---------------- build helper/master mapping ---------------- */
    std::vector<vector<int>> helper(world_size);
    std::vector<int> master(world_size, -1);

    std::queue<int> zero_ranks;
    priority_queue<pair<int, int>> qu;
    for(int i=0;i<world_size;++i){
      qu.push({taskcount[i], i});
      if(taskcount[i]==0) zero_ranks.push(i);
    }
    while (qu.size() > 0) {
      auto p = qu.top();
      qu.pop();
      if (p.first < task_threshold)
        break;
      int i = p.second;
      if (master[i] != -1)
        continue;
      if (zero_ranks.size() > 0) {
        int j = zero_ranks.front();
        zero_ranks.pop();
        master[j] = i;
        helper[i].push_back(j);
        qu.push({taskcount[i] / (helper[i].size() + 1), i});
      }
      else{
        break;
      }
    ...
    mytasks.reserve((NN * nvar) / world_size + nvar);
    for (int j = 0; j < NN; ++j) {
      int rank = belongings[j]->rank;
      for (int k = 0; k < nvar; ++k) {
        int t = k % (helper[rank].size() + 1);
        int task_rank;
        if(t==0){
          task_rank = rank;
        }
        else{
          task_rank = helper[rank][t-1];
          if(myrank == rank)
            sendtasks.push_back({j, k, task_rank});
        }
        if(task_rank == myrank)
          mytasks.push_back({j, k, task_rank});
      }
    }

    /* ---------------- local buffer ---------------- */
    std::vector<double> shellf(NN * nvar, 0.0);

    /* ============================================================
       CASE 1 : I am master (need to send data to helper)
       ============================================================ */
    std::vector<MPI_Request> send_reqs;
    send_reqs.reserve(sendtasks.size() * 4);
    if (helper[myrank].size() > 0) {

      std::set<std::pair<Block *, int>> sent;

      for (auto &task : sendtasks) {

        Block *bp = belongings[task.j];
        int sgfn = allvars[task.k]->sgfn;

        if (!sent.insert({bp, sgfn}).second)
          continue;
        ...
        MPI_Isend(bp->fgfs[sgfn], size, MPI_DOUBLE, task.rank, 0,
                 MPI_COMM_WORLD, &req);
        send_reqs.push_back(req);

        MPI_Isend(bp->X[0], nx, MPI_DOUBLE, task.rank, 0, MPI_COMM_WORLD, &req);
        send_reqs.push_back(req);

        MPI_Isend(bp->X[1], ny, MPI_DOUBLE, task.rank, 0, MPI_COMM_WORLD, &req);
        send_reqs.push_back(req);

        MPI_Isend(bp->X[2], nz, MPI_DOUBLE, task.rank, 0, MPI_COMM_WORLD, &req);
        send_reqs.push_back(req);
      }
    }

    /* ============================================================
       CASE 2 : I am helper (receive data)
       ============================================================ */
    std::map<std::pair<Block *, int>, std::vector<double>> recv_fgfs;

    std::map<std::pair<Block *, int>, std::array<std::vector<double>, 3>>
        recv_X;

    if (master[myrank] != -1) {
      std::vector<MPI_Request> recv_reqs;
      recv_reqs.reserve(mytasks.size() * 4);

      int master_rank = master[myrank];

      for (auto &task : mytasks) {
        Block *bp = belongings[task.j];
        int sgfn = allvars[task.k]->sgfn;
        auto key = std::make_pair(bp, sgfn);

        if (recv_fgfs.find(key) != recv_fgfs.end())
          continue;
        ...
        recv_fgfs[key].resize(size);
        recv_X[key][0].resize(nx);
        recv_X[key][1].resize(ny);
        recv_X[key][2].resize(nz);

        MPI_Request req;

        MPI_Irecv(recv_fgfs[key].data(), size, MPI_DOUBLE, master_rank, 0,
                 MPI_COMM_WORLD, &req);
        recv_reqs.push_back(req);

        MPI_Irecv(recv_X[key][0].data(), nx, MPI_DOUBLE, master_rank, 0,
                 MPI_COMM_WORLD, &req);
        recv_reqs.push_back(req);

        MPI_Irecv(recv_X[key][1].data(), ny, MPI_DOUBLE, master_rank, 0,
                 MPI_COMM_WORLD, &req);
        recv_reqs.push_back(req);

        MPI_Irecv(recv_X[key][2].data(), nz, MPI_DOUBLE, master_rank, 0,
                 MPI_COMM_WORLD, &req);
        recv_reqs.push_back(req);
      }
    ...
    /* ============================================================
       COMPUTE PHASE (common)
       ============================================================ */
#pragma omp parallel for schedule(dynamic)
    for (int i = 0; i < (int)mytasks.size(); ++i) {
      const Task &task = mytasks[i];
      Block *bp = belongings[task.j];
      int sgfn = allvars[task.k]->sgfn;
      ...
      if (master[myrank] != -1) {
        auto key = std::make_pair(bp, sgfn);
        X0 = recv_X[key][0].data();
        X1 = recv_X[key][1].data();
        X2 = recv_X[key][2].data();
        fgfs = recv_fgfs[key].data();
      } else {
        X0 = bp->X[0];
        X1 = bp->X[1];
        X2 = bp->X[2];
      ...
      f_global_interp(bp->shape, X0, X1, X2, fgfs,
                      shellf[task.j * nvar + task.k], XX[0][task.j],
                      XX[1][task.j], XX[2][task.j], ordn, allvars[task.k]->SoA,
                      Symmetry);
    }
    ...
    /* ---------------- reduction ---------------- */
    MPI_Allreduce(shellf.data(), Shellf, NN * nvar, MPI_DOUBLE, MPI_SUM,
                  MPI_COMM_WORLD);
  }
};
...
{
  ShellfScheduler scheduler(this, NN, XX);
  // scheduler.view();
  scheduler.run2(Shellf, VarList, Symmetry);
}
void Patch::Interp_Points(MyList<var> *VarList,
                          int NN, double **XX,
